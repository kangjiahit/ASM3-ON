...
DON_1<-function(KO,KS,KUAP,KBAP,KSTO,uH,KNH,KALK,KNO,
  ...
  r4=(-p21-p31+kUAPO*p4+kUAPNO*p5
       +kUAPAO*p10-p22-p32
       +kBAPO*p6+kBAPNO*p7
       +kBAPAO*p11+kBAPANO*p12)*07-p13
...
DON_2 <- function(X, t =T) {
  
  out <- matrix(nrow = nrow(X), ncol = length(t))
  for (i in 1:nrow(X)) {
    out[i, ] <- DON_1(X$KO[i],X$KS[i],X$KUAP[i],X$KBAP[i],
...
DON.seq.sobol <- multisensi(design = sobol, model = DON_2,
...
DON.seq.morris<-multisensi(design = morris,model=DON_2,reduction = NULL,analysis = analysis.sensitivity,
...
library(ggplot2)
library(ggrepel)
data_plot_sobol<-as.data.frame(cbind(rownames(DON.seq.sobol$mSI),DON.seq.sobol$mSI$GSI))
...
data_plot_morris<-as.data.frame(cbind(rownames(DON.seq.morris$mSI),DON.seq.morris$mSI$t1))
...
save.image(file = "DON_model.RData")
